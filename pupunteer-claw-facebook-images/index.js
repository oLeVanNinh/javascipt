...
(async () => {
  ...
  await page.goto("https://www.facebook.com/mua.linh.758/photos_all")
...
//    await page.screenshot({path: `./images/example${random}d.png`});
    // await page.goto(link);
    // await page.screenshot({path: `./images/example${Math.floor((Math.random()*1000).toString())}`})
...
async function getSrc(page) {
  let srcs = await page.evaluate(() => {
    const links = document.querySelectorAll('ul > li.fbPhotoStarGridElement > a');
